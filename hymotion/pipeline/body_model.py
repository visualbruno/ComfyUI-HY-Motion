...
def simple_lbs(v_template, rot_mats, joints, parents, skin_weights, skin_indices):
    ...
    joints_homo = F.pad(joints, [0, 1], value=0).to(dtype)  # (J, 4)
    ...
    skin_weights_expanded = skin_weights.to(dtype).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)  # (1, V, 4, 1, 1)
    ...
    # Apply to vertices
    v_homo = F.pad(v_template, [0, 1], value=1.0).to(dtype)  # (V, 4)
...
class WoodenMesh(torch.nn.Module):
    ...
    def __init__(self, model_path=None):
        ...
        if model_path is None:
            # Default path relative to this file (hymotion/pipeline/body_model.py)
            # scripts is at the root, which is 3 levels up from this file
            base_path = Path(__file__).resolve().parent.parent.parent
            model_path = base_path / "scripts" / "gradio" / "static" / "assets" / "dump_wooden"
...
def construct_smpl_data_dict(
    ...
    dump = {
        "betas": betas.float().cpu().numpy() if betas is not None else np.zeros((1, 16)),
        "gender": gender,
        "poses": angle_axis.float().cpu().numpy().reshape(angle_axis.shape[0], -1),
        "trans": transl.float().cpu().numpy(),
        "mocap_framerate": 30,
        "num_frames": angle_axis.shape[0],
        "Rh": angle_axis.float().cpu().numpy().reshape(angle_axis.shape[0], -1)[:, :3],
    }
    return dump
