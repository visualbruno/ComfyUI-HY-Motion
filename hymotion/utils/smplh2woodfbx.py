...
def _animateTranslationKeyFrames(animLayer, node, translations, frameDuration):
    ...
    if isinstance(translations, torch.Tensor):
        translations = translations.float().cpu().numpy()
...
def _convert_smplh_to_woodfbx(
    ...
    rot_matrices = angle_axis_to_rotation_matrix(poses).float().cpu().numpy()

    # Prepare translation data
    trans = npz_data["trans"]
    if isinstance(trans, torch.Tensor):
        trans = trans.float().cpu().numpy()
...
class SMPLH2WoodFBX:
    ...
    def __init__(
        self,
        template_fbx_path: Optional[str] = None,
        ...
        if template_fbx_path is None:
            ...
            # assets is at the root, which is 2 levels up from this file (hymotion/utils/smplh2woodfbx.py)
            base_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            template_fbx_path = os.path.join(base_path, "assets", "wooden_models", "boy_Rigging_smplx_tex.fbx")
